...
/// </summary>
public class RightWindowController : MonoBehaviour
{
    ...
    [Header("Derived Door State")]
    [SerializeField] private float openStateThresholdDeg = 3f;
    [SerializeField] private float ventActiveAtDeg = 30f;

    public float CurrentDoorAngle => _doorAngle;
    public float Open01 { get; private set; }
    public bool IsOpen { get; private set; }
    public bool IsVenting { get; private set; }

    public event Action<RightWindowController, float, float> DoorAngleChanged;
    public event Action<RightWindowController, float, float> OpenAmountChanged;
    public event Action<RightWindowController> DoorOpened;
    public event Action<RightWindowController> DoorClosed;
    public event Action<RightWindowController, bool> VentingStateChanged;

    private float _publishedDoorAngle;
    private float _publishedOpen01;
    private bool _publishedIsOpen;
    private bool _publishedIsVenting;
    ...
    void Start()
    {
        ...
        SyncDoorDerivedState(force: true);
    }
    ...
    // Cập nhật trạng thái phụ thuộc của cửa (Open01, IsOpen, IsVenting) dựa trên góc cửa hiện tại.
    private void SyncDoorDerivedState(bool force = false)
    {
        float openedDeg = Mathf.Abs(Mathf.DeltaAngle(closedAngle, _doorAngle));
        float maxOpenedDeg = Mathf.Max(0.0001f, Mathf.Abs(Mathf.DeltaAngle(closedAngle, openAngle)));

        float newOpen01 = Mathf.Clamp01(openedDeg / maxOpenedDeg);
        bool newIsOpen = openedDeg >= openStateThresholdDeg;
        bool newIsVenting = openedDeg >= ventActiveAtDeg;

        Open01 = newOpen01;
        IsOpen = newIsOpen;
        IsVenting = newIsVenting;

        if (force || Mathf.Abs(_doorAngle - _publishedDoorAngle) > 0.01f)
        {
            float old = _publishedDoorAngle;
            _publishedDoorAngle = _doorAngle;
            DoorAngleChanged?.Invoke(this, old, _doorAngle);
        }

        if (force || Mathf.Abs(newOpen01 - _publishedOpen01) > 0.001f)
        {
            float old = _publishedOpen01;
            _publishedOpen01 = newOpen01;
            OpenAmountChanged?.Invoke(this, old, newOpen01);
        }

        if (force || newIsOpen != _publishedIsOpen)
        {
            _publishedIsOpen = newIsOpen;
        ...
        if (force || newIsVenting != _publishedIsVenting)
        {
            _publishedIsVenting = newIsVenting;
            VentingStateChanged?.Invoke(this, newIsVenting);
        }
    }
}