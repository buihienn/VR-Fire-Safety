...
namespace Oculus.Interaction
{
    ...
    public class OneGrabKnobTransformer : MonoBehaviour, ITransformer
    {
        ...
        public void BeginTransform()
        {
            ...
            Vector3 rotationAxis = -pivot.TransformDirection(Vector3.right);
        ...
        public void UpdateTransform()
        {
            ...
            Vector3 targetDir = Vector3.ProjectOnPlane(targetPose.forward, rotationAxis);
            Vector3 previousDir = Vector3.ProjectOnPlane(previousPose.forward, rotationAxis);
            if (targetDir.sqrMagnitude < 1e-6f || previousDir.sqrMagnitude < 1e-6f)
            {
                targetDir = Vector3.ProjectOnPlane(targetPose.up, rotationAxis);
                previousDir = Vector3.ProjectOnPlane(previousPose.up, rotationAxis);
            }

            var angleDelta = Vector3.SignedAngle(targetDir, previousDir, rotationAxis);
